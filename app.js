...
async function unlockAndRevealCloud() {
  ...
  const { data, error } = await db.rpc("check_reveal_passcode", {
...
async function clearSelectedExperience() {
  ...
  const confirmed = window.confirm("Clear all responses for the selected experience?");
  if (!confirmed) return;
...
async function deleteSelectedExperience() {
  ...
  const confirmed = window.confirm(`Delete "${target?.name || "this experience"}" and all its responses?`);
  if (!confirmed) return;
